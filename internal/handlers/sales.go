...
func RecentSalesHandler(w http.ResponseWriter, r *http.Request) {
	...
	// ✅ Get shop_id from cookie or query
	shopID := claims.ShopID
	if shopID == 0 {
		shopID = 1
	}

	// ✅ Get sales for this shop and company
	sales, err := db.GetRecentSalesForShopAndCompany(db.GetDB(), shopID, claims.CompanyID, 20)
	if err != nil {
		log.Printf("❌ Error getting recent sales: %v", err)
